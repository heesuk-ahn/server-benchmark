...
그러나 하나의 요청이 하나의 스레드를 차지하게 되면, 해당 스레드는 더 많은 일을 할 수 가 없어진다.
 스레드가 Block 함수를 만나게 되면 스레드는 `BLOCK STATUS` 에 빠지게 되고, 그때 바로 `스레드 컨텍스트 스위칭`이
발생하게 된다. 그리고 블락 상태에 대기중인 스레드는 다른 일을 하지 못하고 해당 함수가 종료 될 때까지, `가만히 기다리게 된다.`
이러한 현상이 지속되면, 일을 할 수 있는 스레드가 병목 지점에서 대기하게 되고, 결국 스레드 풀 헬 현상까지 이어지게 된다.

(이미지 추가 - 링크드인 스레드풀 헬)
(이미지 추가 - 요청에 따른 JAVA IO 네트워크 소켓 처리)
(이미지 추가 - 요청에 따른 JAVA IO 네트워크 소켓 처리 멀티 스레드 방식)

### 설정
--

톰캣의 요청 스레드 풀은 하나로 고정한다.

```aidl
[tomcat server thread setting]
...
[!ASYNC-RestTemplate](/ASYNC-RestTemplate.png)
